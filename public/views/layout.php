        ...
        <section>
            ...
            <div class="admin-grid">
                <form method="post" action="<?= e(route_url('admin_save_login_cover')) ?>" enctype="multipart/form-data" class="stack-form">
                    ...
                    <p class="helper-text">Admin e Super Admin possono caricare una nuova immagine di sfondo per la pagina di accesso.</p>
                    <label>Nuova immagine sfondo login
                        <input type="file" name="login_cover_image" accept="image/jpeg,image/png,image/webp,image/gif">
                    </label>
                    <button type="submit">Carica sfondo login</button>
                </form>
                <div class="stack-form">
                    <h3>Immagine attuale pagina login</h3>
                    <?php if ($loginCoverImage): ?>
                        <img src="<?= e(media_url($loginCoverImage)) ?>" alt="Sfondo pagina login" class="admin-preview">
                        <p class="helper-text">Questa immagine viene mostrata sullo sfondo della pagina di login.</p>
                        <form method="post" action="<?= e(route_url('admin_reset_login_cover')) ?>" class="stack-form small-form">
                            <button type="submit" class="secondary-button">Elimina foto login</button>
                        </form>
                    <?php else: ?>
                        <p class="helper-text">Nessuna immagine personalizzata presente. La pagina login usa lo sfondo neutro predefinito.</p>
                    <?php endif; ?>
                </div>
            </div>
            <div class="admin-grid">
